...
# EXTRA STAGE / DIFFERENCE: SINGLE STAGE CONVOLUTION INSTEAD 2
...
# EXTRA 1 d conv STAGE / DIFFERENCE: single 1d convolution stage
def one_d_conv_block(in_chan, out_chan, stride=1):
    return nn.Sequential(
        nn.Conv3d(in_chan, out_chan, kernel_size=3, padding=1, stride=stride),
        nn.BatchNorm3d(out_chan),
        nn.ReLU(inplace=True)
...
def one_d_conv_stage(in_chan, out_chan):
    return nn.Sequential(
        one_d_conv_block(in_chan, out_chan),
    )
##################################################################
...
class UNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.enc5 = conv_stage(8, 12)
        self.pool = nn.MaxPool3d(2, 2)

        self.decs4 = extra_conv_stage(36, 8)
        self.dec4 = conv_stage(8, 8)
        self.decs3 = extra_conv_stage(36, 8)
        self.dec3 = conv_stage(8, 4)
        self.decs2 = extra_conv_stage(32, 4)
        self.dec2 = conv_stage(4, 4)
        self.decs1 = extra_conv_stage(36, 4)
    ...
    def forward(self, x):
        ...
        dec4_1 = torch.cat((enc4, enc3dec4, enc2dec4, enc1dec4,
        ...
        dec3_1 = torch.cat(
            (enc3, enc2dec3, enc1dec3, F.interpolate(dec4, enc3.size()[2:], mode='trilinear', align_corners=False)),
            1)  # 176
        enc5dec3 = F.interpolate(enc5, enc3.size()[2:], mode='trilinear', align_corners=False)
        dec3_1_1 = torch.cat((enc5dec3, dec3_1), 1)  # 304
        # 304

        decs3 = self.decs3(dec3_1_1)
        dec3 = self.dec3(decs3)

        dec2_1 = torch.cat(
            (enc2, enc1dec2, F.interpolate(dec3, enc2.size()[2:], mode='trilinear', align_corners=False)), 1)
